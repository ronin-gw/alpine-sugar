FROM adoptopenjdk/openjdk14:alpine as jre-builder

RUN jlink --compress=2 --module-path /opt/java/openjdk/jmods --add-modules java.base,java.desktop,java.logging --output /jre-min
...
RUN apk add --update --no-cache fontconfig ttf-dejavu \
    && apk add --no-cache --virtual tzdata curl binutils zstd \
    && GLIBC_VER="2.31-r0" \
...
COPY --from=jre-builder /jre-min /opt/java/openjdk
